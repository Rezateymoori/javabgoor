...
# پیدا کردن خودکار یک مدل متنی فعال
def get_available_model():
    models = genai.list_models()
    for m in models:
        # بررسی اینکه نام مدل شامل "gemini" باشد و قابلیت متنی داشته باشد
        capabilities = [c.lower() for c in getattr(m, "capabilities", [])]
        if "gemini" in m.name.lower() and "text" in capabilities:
            return m.name
